...
def go():
    ...
    headerrow = ["File", "Odom Distance (m)", "Goal Distance (m)", "Time to waypoint (s)", "Average Velocity Over Waypoint (m/s)",
                 "Standardized Waypoint Deviation (m/m)", "Lidar Range", "Planner", "Door Probability", "Laser Noise", "Gyro Noise", "Accel Noise", "Dijkstra"]
    ...
    with open(os.path.dirname(os.path.realpath(__file__))[:-8] + "/csv/result.csv", 'a') as csvfile:
        ...
        filewriter.writerows(np.column_stack(
            (bagFilecolumn, odom, goal, result, avgvel, stdwpt, lidarrange, planner, door, lasernoise, imugyronoise, imuaccelnoise, dijkstra)))
    bag.close()
